...
echo -e "${BLUE}🚀 开始 Excalidraw Plus 本地开发环境部署...${NC}"
echo -e "${YELLOW}   使用完整的 Docker 容器（包含 MySQL）${NC}"
echo ""
...
for i in {1..10}; do
    if docker-compose -f docker-compose.dev.yml exec -T mysql mysqladmin ping -h localhost -u excalidraw -pexcalidraw_password &> /dev/null; then
        echo -e "${GREEN}✅ MySQL 已就绪！${NC}"
        break
    fi
    if [ $i -eq 10 ]; then
        echo -e "${RED}❌ MySQL 启动超时${NC}"
        echo -e "${YELLOW}请检查日志: docker-compose -f docker-compose.dev.yml logs mysql${NC}"
        exit 1
    fi
...
echo -e "   - 查看数据库: ${YELLOW}docker-compose -f docker-compose.dev.yml exec mysql mysql -u excalidraw -p excalidraw_plus${NC}"
